...
public class TodoController {
  ...
  public void addNewTodo(Context ctx) {
    Todo newTodo = ctx.bodyValidator(Todo.class)
      ...
      .check(todo -> todo.status == true || todo.status == false ) // Verify that the provided status is a boolean
      .check(todo -> todo.category !=null && todo.category.length() > 0) // Verify that the todo has a category that is not blank
      .check(todo -> todo.body != null && todo.owner.length() > 0) // Verify that the todo has a body that is not blank
      .get();
